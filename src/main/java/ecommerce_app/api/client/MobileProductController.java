...
import ecommerce_app.modules.product.model.dto.response.MobileProductResponse;
...
@RestController
@RequestMapping("/api/mobile/v1/products")
...
public class MobileProductController {
     ...
     * GET /api/mobile/v1/products?page=0&size=20&sort=createdAt,desc
     */
    @GetMapping
    public ResponseEntity<ApiResponse<PageResponse<MobileProductListResponse>>> getProducts(
        ...
        Page<Product> productPage = productService.findAll(pageable, categoryId, isFeature, hasPromotion, search);

        // Map to DTOs
        List<MobileProductListResponse> content = productPage.getContent().stream()
                .map(productMapper::toListResponse)
                .collect(Collectors.toList());

        // Create paginated response
        PageResponse<MobileProductListResponse> pageResponse = PageResponse.of(
                content,
    ...
    /**
     * Get single product detail by UUID
     * GET /api/mobile/v1/products/uuid/{uuid}
     */
    @GetMapping("/uuid/{uuid}")
    public ResponseEntity<ApiResponse<MobileProductResponse>> getProductByUuid(@PathVariable UUID uuid) {
     ...
     * GET /api/mobile/v1/products/featured?size=10
     */
    @GetMapping("/featured")
    public ResponseEntity<ApiResponse<List<MobileProductListResponse>>> getFeaturedProducts(
        ...
        List<MobileProductListResponse> response = products.stream()
                .map(productMapper::toListResponse)
                .collect(Collectors.toList());
     ...
     * GET /api/mobile/v1/products/promotions?page=0&size=20
     */
    @GetMapping("/promotions")
    public ResponseEntity<ApiResponse<PageResponse<MobileProductListResponse>>> getPromotionalProducts(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
     ...
     * GET /api/mobile/v1/products/category/{categoryId}?page=0&size=20
     */
    ...
    /**
     * Search products
     * GET /api/mobile/v1/products/search?q=laptop&page=0&size=20
     */
    @GetMapping("/search")
